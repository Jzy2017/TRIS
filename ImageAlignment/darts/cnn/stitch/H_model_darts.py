...
import time
from darts.cnn.model import NetworkBackbone as feature_extractor
...
class RegressionNet(torch.nn.Module):
    def __init__(self, is_training, search_range, warped=False):
        ...
        self.conv2 = torch.nn.Sequential(
            nn.Conv2d(in_channels=self.out[0], out_channels=self.out[1], kernel_size=3, stride=self.stride[1],
                      padding=1),
            nn.BatchNorm2d(self.out[1]),
            nn.ReLU(True))
        self.conv3 = torch.nn.Sequential(
            nn.Conv2d(in_channels=self.out[1], out_channels=self.out[2], kernel_size=3, stride=self.stride[2],
                      padding=1),
    ...
    def forward(self, feature1, feature2):
        ...
        if not self.warped:
            ...
            verti_attn, attn_y = self.vertical_attention(nn.functional.normalize(feature1, dim=1, p=2), feature2,
                                                         position)
            hori_attn, attn_x = self.horizontal_attention(nn.functional.normalize(feature1, dim=1, p=2), feature2,
                                                          position)
...
class H_estimator(torch.nn.Module):
    def __init__(self, batch_size, device, is_training=1):
        ...
        self.feature_ir = feature_extractor(C=16, num_classes=10, layers=4, auxiliary=False, genotype=genotype)  # .cuda()
        self.feature_ir.drop_path_prob = 0.1
        self.feature_vis = feature_extractor(C=16, num_classes=10, layers=4, auxiliary=False, genotype=genotype)  # .cuda()
        self.feature_vis.drop_path_prob = 0.1
