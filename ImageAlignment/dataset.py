...
class Image_UDIS(data.Dataset):
    # "数据集"

    def __init__(self, vis1_path: str, vis2_path: str, is_train=True):
        super(Image_UDIS, self).__init__()
        self.train = is_train
        self.vis1_path, self.vis2_path = vis1_path, vis2_path
        self.vis1s = sorted([x for x in os.listdir(vis1_path) if is_image(x)])
        self.vis2s = sorted([x for x in os.listdir(vis2_path) if is_image(x)])
    ...
    def __getitem__(self, index):
        ...
        vis2 = cv2.imread(os.path.join(self.vis2_path, name))
        if self.train:
            vis1 = cv2.resize(vis1, (128, 128))
            vis2 = cv2.resize(vis2, (128, 128))
        # shift=np.reshape(shift,(4,2))
        height = vis1.shape[0]
        width = vis1.shape[1]
        size = np.array([width, height], dtype=np.float32)
        ...
        return vis1, vis2, size, name.split('/')[-1]

    def __len__(self):
        return len(self.vis1s)


class Image_COCO_Test(data.Dataset):
    # "数据集"
