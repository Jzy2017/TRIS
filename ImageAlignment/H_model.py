...
class H_estimator(torch.nn.Module):
    ...
    def forward(self, inputs_vis, gt=None,is_test=False,patch_size=128.):
        ...
        if is_test:
            #return vis_net1_f + vis_net2_f #+ vis_net3_f  #2 scale
            #return vis_net2_f + vis_net3_f #1 scale
            return vis_net1_f + vis_net2_f + vis_net3_f  #3 scale
        """
        ###############################################################################
...
class H_joint_out(torch.nn.Module):
    ...
    def forward(self, offset, size, viss):
        ############### build_model ###################################
        offset_out = offset
        #offset_out = torch.unsqueeze(offset_out, 2)#*128
...
class H_estimator_2(torch.nn.Module):
    def __init__(self, batch_size, device, is_training=1):
        super().__init__()
        self.device = device
        self.feature_vis = feature_extractor()  # .cuda()
